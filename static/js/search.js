...
class SearchPage {
  ...
  renderSearchResults(data) {
    ...
    mobileToggle.innerHTML = 'Filters <span class="toggle-chevron">&#9660;</span>';
  ...
  renderAdvancedFiltersToggle() {
    ...
    if (this.advancedFiltersActive) {
      ...
      label.textContent = 'Activate advanced filters';
    ...
    btn.textContent = 'Activate extra filters (repository, date, etc.)';

    const note = document.createElement('span');
    note.className = 'advanced-filters-note';
    note.textContent = ' \u2014 could take extra loading time';
    btn.appendChild(note);

    btn.addEventListener('click', async () => {
      btn.disabled = true;
      btn.textContent = 'Loading additional filters\u2026';
      try {
        await this.ensurePagefindLoaded();
  ...
  renderNoResults(data) {
    ...
    if (data && data.emptyReason === 'only-not') {
      msg.textContent = 'Add at least one non-NOT term.';
    } else {
    ...
    if (hasFilters) {
      ...
      suggestion.textContent = 'Try clearing the filters or modifying the query.';
