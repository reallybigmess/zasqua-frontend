...
(function () {
  ...
  InfiniteBipartiteExplorer.prototype.init = async function (opts) {
    ...
    requestAnimationFrame(function () {
      ...
      requestAnimationFrame(function () {
        ...
        if (self.graphInstance) self.graphInstance.zoom(2);
      });
    });
  ...
  InfiniteBipartiteExplorer.prototype.showEntityHoverTooltip = function (node) {
    ...
    html += escapeHtml(typeName[node.entity_type] || node.entity_type || 'Entity');
    html += '</span></div>';
    html += '<div class="graph-tooltip-name">' + escapeHtml(node.label || node.id) + '</div>';
    html += '<div class="graph-tooltip-meta">' + (node.linked_count || '?') + ' linked documents</div>';
  ...
  InfiniteBipartiteExplorer.prototype.showOverflowHoverTooltip = function (node) {
    ...
    var html = '';
    html += '<div class="graph-tooltip-name">' + (node.hiddenCount || 0) + ' more documents</div>';
    html += '<div class="graph-tooltip-meta">Click to load the next batch</div>';
  ...
  InfiniteBipartiteExplorer.prototype.showEntityTooltip = function (node) {
    ...
    var typeName = { person: 'Person', corporate_body: 'Corporate body', corporate: 'Corporate body', family: 'Family' };
    ...
    badge.textContent = typeName[node.entity_type] || node.entity_type || 'Entity';
    ...
    meta.textContent = (node.linked_count || '?') + ' linked documents';
    ...
    if (!isFocal) {
      ...
      refocusBtn.textContent = 'Select and display links';
  ...
  InfiniteBipartiteExplorer.prototype.showDocumentTooltip = function (node) {
    ...
    this.lookupDocEntities(node.reference_code).then(function (codes) {
      ...
      span.textContent = 'Connected to ' + newCodes.length + ' more entit' + (newCodes.length !== 1 ? 'ies' : 'y') + '. ';
  ...
  InfiniteBipartiteExplorer.prototype.renderLegend = function () {
    ...
    el.innerHTML = [
      legendItem('#8B2942', 'filled', 'Person'),
      legendItem('#6666BB', 'filled', 'Corporate body / family'),
      legendItem(DOC_COLOR, 'filled', 'Item with connections'),
      legendItem(DOC_COLOR, 'hollow', 'Item with no connections')
    ].join('');
  ...
  InfiniteBipartiteExplorer.prototype.loadMoreDocs = function (overflowNode) {
    ...
    if (overflowNode.hiddenCount <= 0) {
      ...
      overflowNode.label = '+' + overflowNode.hiddenCount + ' documents';
      this._redraw();
    }
