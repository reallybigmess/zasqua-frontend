 ...
 *
 * Drives the place explorer at `/lugares/`: a Pagefind-powered search
 * over the places index, a MapLibre + Protomaps terrain basemap with
 * clustered markers, paginated results list, sidebar facets
 * (`place_type` / `has_coordinates` / `has_authority`), filter pills,
 * URL state sync, sort toggle, selected-place card, and a viewport-
 * bound filter toggle that re-derives counts from the in-memory
 * places list when active. Pagefind is a client-side search engine
 * that ships a small WebAssembly bundle plus a JSON index built at
 * build time, so the search runs entirely in the browser with no
 * server round-trip. The file is loaded as a classic `<script>` tag
 * (no ES module imports) and the `PlaceExplorer` class self-instantiates
 * via the bottom-of-file `DOMContentLoaded` listener when an
 * `#place-explorer` host is present.
 *
 * Pipeline context:
 *   The template `lugares.njk` provides the full explorer-grid layout
 *   and an `#place-explorer` host with `data-place-types`,
 *   `data-maptiler-key`, and `data-maptiler-style-id` datasets.
 *   Build-time inputs are the
 *   `/pagefind-places/` bundle (Pagefind, written by `build.sh`
 *   Stage 7) and `/data/place-index.json` (map coordinates). Run-time
 *   inputs are the user's URL params, search/facet clicks, map drags.
 *   Outputs are DOM updates inside the template slots:
 *     - #place-search-input    — search input field
 ...
 *
 * Phase 14 additions (v1.1.0):
 *   - `selectFacetCounts(result, facetKey, value, activeFilters)` is
 *     inlined here byte-identical with `static/js/search.js` and
 *     `static/js/entity-explorer.js` per D-10 (no shared module — each
 *     `<script>`-loaded file carries its own copy). The helper is the
 *     canonical Pagefind facet-count selector confirmed by plan 14-01
 *     (D-09): facet keys with an active filter read from
 *     `result.totalFilters`, cross-facet keys from `result.filters`.
 *     This closes Bug 1 on `/lugares/`.
 *   - The former single-source `scopedFilters` consumer (which
 *     pulled `searchResult.filters` with a globalFilters fallback,
 *     around line ~561) now computes a
 *     per-value `scopedCounts` map via `_buildScopedFacetCounts` and
 *     feeds it to `renderFacets`, preserving the existing
 *     `{ facetKey: { value: count } }` shape the three facet-group
 *     blocks already consume. The viewport-bound code path
 *     (`vpFacets`) is untouched — it derives counts from the
 *     in-memory places list, not from Pagefind, and so was never
 *     subject to the `filters` / `totalFilters` split.
 *   - The bottom-of-file `DOMContentLoaded` listener is now wrapped in
 *     a `typeof document` guard so the file can be loaded under
 *     Node/Vitest (which has no DOM) without throwing — matches the
 *     pattern in `static/js/search.js`. The conditional CommonJS
 *     export keeps the three Phase 14 client files symmetric.
 *
 * Note on style: this file uses ES5 `var`-scope idioms throughout the
 * controller methods. The Phase 14 additions preserve that — the
 * helper itself is a self-contained `const`-internally function body
 * (matching the byte-identical contract); the consumer rewire stays
 * `var`-scoped.
 *
 * Phase 15.3-02 changes (v1.2.0):
 *   - Ported the Phase 15.1-10 active-single-value short-circuit into
 *     `selectFacetCounts` (D-10 byte-equivalence with
 *     `static/js/search.js` + `static/js/entity-explorer.js`). Fixes
 *     the user-reported bug where selecting a single `place_type=city`
 *     renders the restore badge as "Lugar poblado (0)" instead of the
 *     scoped total (~3,437 places). CONTEXT scope row #2; origin commit
 *     `cc7a2cd` (Phase 15.1-10); entity port commits `6feb63b` /
 *     `c0cc071` / `0cee60b` (Phase 15.2-02). Local variable rename:
 *     `activeForFacet` → `activeInKey` (canonical name in search.js +
 *     entity-explorer.js). JSDoc extended with place-side edge-case
 *     description. D-10 byte-equivalence proof: 3-way diff across
 *     search.js / entity-explorer.js / place-explorer.js is empty.
 *   - Moved `this.pagefind.filters()` off the init critical path into a
 *     fire-and-forget `.then()` chain (mirrors `static/js/search.js:
 *     549-553` commit `bf6c1f2` / entity-explorer.js post-15.2-02).
 *     First paint no longer blocks on the ~7-13 s `filters()` call.
 *     Every synchronous read of `this.globalFilters` confirmed null-safe
 *     via grep; constructor default `{}` makes the existing
 *     `|| this.globalFilters` patterns safe without additional guards.
 *     CONTEXT scope row #7.
 *   - Ghost-pill fix: `clearBtn` moved out of `pillsEl` (the
 *     `.active-filters` flex row) into a persistent sibling element
 *     `this.clearBtnEl` created in `buildDOM()`. This removes the
 *     visual ambiguity where the gray pill-shaped button appeared
 *     alongside the active filter pills as if it were a second pill.
 *     Root cause documented in `.planning/phases/15.3-lugares-parity-
 *     performance/15.3-02-GHOST-PILL-TRACE.md` (D-05 contract artefact);
 *     Hypothesis 1 confirmed — `clearBtn` was a flex sibling inside
 *     `pillsEl`. CONTEXT scope row #8.
 *
 * Phase 15.3-03 changes (v1.3.0):
 *   - init() now fetches /lugares-facets.json via Promise.allSettled
 *     before the pagefind.filters() warm chain. Cold landing render of
 *     /lugares/ paints real sidebar counts immediately rather than
 *     waiting ~7-13 s for the warm chain. Mirrors the
 *     /buscar/ landing-sidecar consumer pattern from search.js:497-527
 *     (15.1-05). Non-fatal on fetch failure — warm chain recovers.
 *     CONTEXT scope rows #5 and #6.
 *
 * Phase 15.3-04 changes (v1.4.0):
 *   - Port PIVOT_KEYS (3 keys, alphabetical: has_authority,
 *     has_coordinates, place_type) + buildPivotScopedFiltersPure from
 *     static/js/entity-explorer.js:218-313 (origin commits aa10e9d /
 *     72a82aa / 130961b — 15.2-04 close-out). Two year-passthrough
 *     lines removed (places carry no year filter). Closes over the
 *     local 3-key PIVOT_KEYS binding automatically.
 *   - PlaceExplorer.prototype.buildPivotScopedFilters class-method
 *     added with the places state-shape adapter: state.type →
 *     place_type, state.hasCoords === true → has_coordinates: ['true'],
 *     state.hasAuthority === true → has_authority: ['true'].
 *   - Constructor initialises this.pivots = null and this.triples =
 *     null alongside the existing this.globalFilters = {} init.
 *   - init() Promise.allSettled extended from 1-entry (15.3-03) to
 *     3-entry: facets + pivots + triples in one parallel fetch.
 *     Non-fatal on each rejection.
 *   - search() gains a pre-Pagefind synchronous sidebar render from
 *     pivotScoped when buildPivotScopedFilters() returns non-null
 *     (N=1 or N=2 active dims with sidecars loaded). No D-04 self-
 *     contradiction window — main-pane total still awaits Pagefind.
 *     Post-Pagefind renderFacets overwrites the pre-Pagefind sidebar
 *     with the authoritative shape. CONTEXT scope row #6, §D-01,
 *     §D-04. PATTERNS.md §Fix #7 + §Fix #8.
 *   - CommonJS export footer extended to expose buildPivotScopedFiltersPure
 *     + PIVOT_KEYS for Vitest coverage.
 *   - D-10 byte-equivalence (2-way diff): entity-explorer.js consumer
 *     ↔ place-explorer.js consumer — identical modulo key names.
 *
 * Phase 15.3-06 changes (v1.5.0):
 *   - Cold-init bug fix: `this.search()` was called only inside the
 *     `this.map.on('load', …)` callback at initMap():724. When the
 *     MapTiler Topo style fails to load (network failure, origin
 *     restriction, timeout), MapLibre emits 'error' instead of 'load',
 *     so the callback never fires and pills / results / facets stay
 *     empty indefinitely. Root cause: H1 — confirmed by Chrome CDP
 *     Fetch interception (style.json blocked → AJAXError → 'load'
 *     never fires; pills=0 results=0 facets=0; popstate dispatch
 *     triggers full correct render). See `.planning/phases/
 *     15.3-lugares-parity-performance/15.3-06-COLD-INIT-TRACE.md`
 *     §E for full hypothesis table.
 *   - Minimal patch: add one unconditional `this.search()` call in
 *     `init()` immediately after `this.initMap()`. The existing
 *     `this.search()` inside the `map.on('load', …)` callback is
 *     LEFT IN PLACE — when the map loads normally it fires a second
 *     search() with `mapReady=true`, pushing marker data to the map.
 *     The double call is safe via the `_searchGen` counter (stale
 *     renders self-cancel) and `updateMap()`'s `if (!this.mapReady)
 *     return;` guard at :1510 (the unconditional call skips the map
 *     update when mapReady is still false). This satisfies CONTEXT
 *     §D-05's "minimal patch addressed at the confirmed hypothesis"
 *     requirement — no blind-fallback setTimeout guard.
 *
 * Phase 15.3-07 changes (v1.6.0):
 *   - Sort-pagination fix attempt: the 'linked' (Documentos desc) sort was
 *     applied after slicing the current page (post-slice sort on hits),
 *     so only the 20 visible items were sorted among themselves. Root
 *     cause identified as H2 (code inspection). Patch: pre-slice
 *     filteredResults sort using allPlaces in-memory lookup keyed by
 *     place_code from result.url. However the patch was a no-op —
 *     see 15.3-08 below.
 *
 * Phase 15.3-08 changes (v1.7.0):
 *   - HOTFIX: 15.3-07's pre-slice sort was a no-op because raw Pagefind
 *     stubs expose no `.url` property (only id, score, words, data()).
 *     `(a.url || '').split('/').filter(Boolean).pop()` always yielded ''
 *     so every comparator returned 0 — no reordering. Additionally,
 *     `pagefind.search({ sort: { linked_count: 'desc' } })` was silently
 *     ignored because the places index never registered `linked_count`
 *     (or `count`) as a sort attribute. Root cause: H6 CONFIRMED —
 *     places `sort:` fn (scripts/generate-pagefind-indices.js:610)
 *     registered only `name`; entities adapter at :571 correctly
 *     registers both `name` and `count`. See 15.3-08-SORT-ROOT-CAUSE-
 *     TRACE.md §B + §C.
 *   - Fix: remove 15.3-07's no-op filteredResults.slice().sort() block.
 *     Build pfSort as `{ count: 'desc' }` when state.sort === 'linked'
 *     (matching the field name the indexer now registers). Pass pfSort
 *     unconditionally to pagefind.search(). No client-side sort needed
 *     once the index exposes the field — Pagefind handles the ordering.
 *     Companion indexer change: generate-pagefind-indices.js v1.6.0
 *     adds `count: String(r._linked_count || 0)` to places sort fn,
 *     mirroring entities. Index rebuild required (place-explorer.js
 *     now passes { count: 'desc' }; the old index has no `count` field).
 *
 * Phase 15.3-09 changes (v1.8.0):
 *   - Sort UI parity with /buscar/: replaces the bare `Nombre | Documentos`
 *     toggle buttons with the /buscar/ sort-wrap DOM shape — `Ordenar por:`
 *     label + pipe-separated buttons each carrying a `<span class="sort-arrow">`
 *     showing ↑ or ↓. State format rotated from the single-field `'name' |
 *     'linked'` to `'field:dir'` (`field` ∈ {'name','linked'}, `dir` ∈
 *     {'asc','desc'}). Constructor default: `'name:asc'`.
 *   - `parseUrlParams` now accepts legacy single-field tokens:
 *       `?sort=`        → `'name:asc'`  (empty or missing)
 *       `?sort=name`    → `'name:asc'`
 *       `?sort=linked`  → `'linked:desc'` (promoted — desc is the natural
 *                         first-click direction for a document-count sort:
 *                         users want the busiest places first)
 *       `?sort=field:dir` → passed through verbatim if valid, else clamped
 *       unknown value   → clamped to `'name:asc'`
 *   - `updateUrl` suppresses the sort param when state is `'name:asc'`
 *     (default — no need to pollute the URL).
 *   - Click handler: inactive Nombre → `name:asc`; inactive Documentos →
 *     `linked:desc` (natural desc on first click — /lugares/ sort convention,
 *     plan 15.3-09); active button toggles direction (asc ↔ desc).
 *   - `pfSort` built by splitting `state.sort` on `:`:
 *       field `name`   → Pagefind sort key `name`
 *       field `linked` → Pagefind sort key `count` (15.3-08 mapping preserved)
 *     Passed unconditionally to `pagefind.search()`.
 *   - Narrative header and version footer bumped to v1.8.0.
 *
 * Satisfies the `/lugares/` half of PAGEFIND-01 (Bug 1 facet-count
 * fix).
 *
 * Version: v1.8.0
 */
 ...
 * `static/js/entity-explorer.js` per D-10 (the three flat `<script>`-
 ...
 *
 * Phase 15.1-10 edge-case correction (active-single-value degenerate),
 * ported into the places copy by Phase 15.3-02 per D-10:
 *   D-09 was designed around OR-group siblings — when the user picks
...
function selectFacetCounts(result, facetKey, value, activeFilters) {
  ...
  // Phase 15.1-10: active-single-value short-circuit. When the queried
...
// Phase 15.3-04 (CONTEXT §D-01): 3-key pivot set for /lugares/. Must
...
/**
 * Phase 15.1-10 + 15.1-11 (ported to /entidades/ in Phase 15.2-04,
 * ported to /lugares/ in Phase 15.3-04):
...
class PlaceExplorer {
  constructor(container) {
    ...
    this.pagefind = null;
    this.globalFilters = {};
    this.pivots = null;       // Phase 15.3-04: pair-wise pivot sidecar (null until fetch)
    this.triples = null;      // Phase 15.3-04: triple-wise pivot sidecar (null until fetch)
  ...
  async init() {
    ...
    try {
      ...
      // pagefind.filters() is NO LONGER awaited here (CONTEXT scope row #7):
    ...
    // Phase 15.3-04 (CONTEXT scope row #6, analog static/js/search.js:497-527 +
    ...
    // Phase 15.3-02 (CONTEXT scope row #7, analog entity-explorer.js post-
    ...
    this.buildDOM();
    this.initMap();
    // Phase 15.3-06 cold-init fix (H1 confirmed — MapTiler 'load' event
  ...
  buildDOM() {
    ...
    // Phase 15.3-02 ghost-pill fix (CONTEXT scope row #8, GHOST-PILL-TRACE.md
  ...
  parseUrlParams() {
    ...
    // Phase 15.3-09: state.sort is now 'field:dir'. Old bookmarks that
  ...
  // Phase 15.3-04 (CONTEXT §D-01 + §D-04 + PATTERNS.md §Fix #7):
  ...
  async search() {
    ...
    // Phase 15.3-04 (CONTEXT scope row #6 + §D-01 + §D-04):
    ...
    // total from a pre-Pagefind path so there is NO D-04 self-contradiction
    ...
    // Build Pagefind sort.
    // Phase 15.3-08 (H6 fix): Pagefind native sort via the `count` field
    // registered by the places indexer (generate-pagefind-indices.js v1.6.0).
    // Phase 15.3-09: state.sort is now 'field:dir'. Split on ':' to get field
    // and direction. Map field 'linked' → Pagefind key 'count' (preserving the
    // 15.3-08 indexer-field mapping). Pass pfSort unconditionally.
    ...
    try {
      ...
      // Phase 14 (D-10): per-value selection via selectFacetCounts.
      ...
      if (this.state.mapBound && this.map && this.mapReady) {
        ...
        var totalPages = Math.ceil(total / this.perPage) || 1;
        if (this.state.page > totalPages) this.state.page = 1;
        // Phase 15.3-08: Pagefind native sort handles ordering for both
  ...
  renderResultsInfo(total, rawTotal) {
    ...
    // Phase 15.3-09: class name is 'sort-wrap'; label 'Ordenar por:'; each
  ...
  // Phase 14 (D-10): build a `{ facetKey: { value: count } }` object
  ...
  renderPills() {
    ...
    // Phase 15.3-02 ghost-pill fix: clearBtnEl (sibling of pillsEl, created
...
// `static/js/search.js` and `static/js/entity-explorer.js` per D-10.
