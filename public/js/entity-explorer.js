 ...
 *
 * Drives the entity explorer at `/entidades/`: a Pagefind-powered search,
 * facet sidebar, and bidirectional sync with the bipartite graph that
 * shares the page. Pagefind is a client-side search engine that ships a
 * small WebAssembly bundle plus a JSON index built at build time, so the
 * search runs entirely in the browser with no server round-trip. This
 * file is loaded as a classic `<script>` tag (no ES module imports) and
 * the `EntityExplorer` class is instantiated from `entidades.njk` so the
 * graph and explorer can coordinate their startup order.
 *
 * Pipeline context:
 *   Build-time input is the `/pagefind-entities/` bundle written by
 *   Pagefind during `build.sh` Stage 7; run-time inputs are the user's
 *   URL parameters, search/facet clicks, and graph viewport changes.
 *   Outputs are DOM updates inside the explorer host
 *   (`#entity-explorer`), the sidebar facet panel
 *   (`#sidebar-facets`), and callbacks the wiring script forwards to
 *   the graph controller (`onEntitySelected`, `onFilterChanged`,
 *   `onFocalRoleFilterChanged`).
 *
 * Phase 14 additions (v1.1.0):
 *   - `selectFacetCounts(result, facetKey, value, activeFilters)` is
 *     inlined here byte-identical with `static/js/search.js` (D-10:
 *     no shared module — each `<script>`-tag-loaded file carries its
 *     own copy). The helper is the canonical Pagefind facet-count
 *     selector confirmed by plan 14-01's DOM diagnostic (D-09): facet
 *     keys with an active filter read counts from `result.totalFilters`
 *     so the active value's restore badge and any same-facet OR-group
 *     siblings stay visible at their real magnitude; cross-facet keys
 *     read from `result.filters`. This closes Bug 1 on
 *     `/entidades/`.
 *   - The two former single-source consumers (which pulled
 *     `search.filters` with a globalFilters fallback in both the
 *     over-threshold "browse prompt" branch and the normal search
 *     return) now compute a per-value scoped-counts object via
 *     `selectFacetCounts` and feed it to `renderSidebarFacets`,
 *     preserving that method's existing `{ facetKey: { value: count } }`
 *     shape contract.
 *   - The `compactMode` constructor probe is wrapped in a
 *     `typeof document` guard so the file can be loaded under
 *     Node/Vitest without a DOM (the class definition itself is pure
 *     and was already require-able). The bottom-of-file conditional
 *     CommonJS export keeps the three Phase 14 client files
 *     symmetric and makes the helper trivially require-able from
 *     `tests/pagefind-facets.test.js` if a future plan adds entity-
 *     specific unit cases.
 *
 * Phase 15.2 changes (v1.2.0):
 *   - Ported the Phase 15.1-10 active-single-value short-circuit into
 *     `selectFacetCounts` (D-10 byte-equivalence with
 *     `static/js/search.js`). Fixes the user-reported bug where
 *     selecting a single `entity_type` value renders the restore
 *     badge with count 0. CONTEXT scope row #2; commit `cc7a2cd`.
 *   - Moved `this.pagefind.filters()` off the init critical path into
 *     a fire-and-forget `.then()` chain (mirrors
 *     `static/js/search.js:549-553` / commit `bf6c1f2`). First paint
 *     no longer blocks on the ~13 s filters() call. Every synchronous
 *     read of the globalFilters object gained an INLINE null-guard so the
 *     pre-warm window is safe — mechanically checkable invariant, no
 *     callee-tolerance exemption (W2 fix). CONTEXT scope row #6.
 *
 * Phase 15.2-04 changes (v1.3.0):
 *   - Ported PIVOT_KEYS + buildPivotScopedFiltersPure + class-method
 *     buildPivotScopedFilters from static/js/search.js (commits
 *     c05141b + 982060f). 4-key pivot set (century, decade,
 *     entity_type, primary_function) per CONTEXT §D-01, byte-equivalent
 *     to ENTITY_PIVOT_FACET_KEYS in scripts/generate-pagefind-indices.js.
 *   - init() parallel-fetches /entidades-pivots.json and
 *     /entidades-triples.json via Promise.allSettled; non-fatal on
 *     either failure (consumer returns null → caller falls back to
 *     global counts / pre-plan overload behaviour). No
 *     /entidades-facets.json equivalent at HEAD — two branches, not
 *     three (PATTERNS §Fix #5 delta 2).
 *   - Overload branch at ~:442-475 gains a pre-Pagefind synchronous
 *     pivot-scoped SIDEBAR render. Main-pane total is DEFERRED to the
 *     Pagefind overload .then() callback to preserve CONTEXT §D-04
 *     no-self-contradiction invariant (W4 fix): user sees authoritative
 *     scoped sidebar counts immediately AND a main-pane pending state
 *     that resolves to the authoritative total when Pagefind lands. No
 *     placeholder / approximate / derived total is ever displayed.
 *     Sites :516-519 (main search) and :701 (clear-filters) unchanged
 *     per PATTERNS §Fix #5 delta 5.
 *
 * Phase 15.2-02 regression fix (v1.3.1):
 *   - Caught in post-structural live probe 2026-04-19: on cold no-filter
 *     landing the 15.2-02 warm-off-critical-path change left init's first
 *     `search()` rendering an empty sidebar + empty `.browse-prompt-count`,
 *     and the stale `#entity-count-live` from the pre-Phase-15.2 HTML
 *     shell never flipped to the canonical 78,245. The pivot sidecar
 *     consumer only covers active-filter windows, so the landing state
 *     had no independent data source. Fix: the `.then()` callback that
 *     stores globalFilters now re-triggers `search()` and `onReady()`
 *     when the sidebar is still empty. Active-filter paths already paint
 *     their own sidebar and the empty-gate skips them, so pivot /
 *     overload renders are untouched. No D-10 byte-equivalence contract
 *     broken (the .then callback already diverges from `static/js/search.js`
 *     because /buscar/ has a `/buscar-facets.json` landing sidecar and
 *     /entidades/ does not yet). Follow-up: adding an
 *     `/entidades-facets.json` landing sidecar (mirroring 15.1-05) would
 *     remove the brief empty-sidebar window while filters() warms.
 *
 * Phase 15.3-10 changes (v1.4.0):
 *   - Sort UI parity with /buscar/ + /lugares/: rotates the bare `Ordenar:`
 *     label to `Ordenar por:` and upgrades each sort-btn from a fixed-value
 *     button to an arrow-carrying, click-toggling button. Each option gains a
 *     `defaultDir` (name=asc, date=asc, count=desc); inactive click applies
 *     defaultDir; active click toggles direction (asc↔desc). Arrow span
 *     (`<span class="sort-arrow">`) shows ↑ or ↓ per current/default dir.
 *     URL param stays `orden`; existing bookmarks unchanged. Block at
 *     :1317-:1357 is the only change — rest of file byte-identical.
 *
 * Satisfies EEXP-01 (search), EEXP-02 (facet filtering), EEXP-03 (list
 * view), and the `/entidades/` half of PAGEFIND-01 (Bug 1 facet-count
 * fix).
 *
 * Version: v1.4.0
 */
 ...
 * `static/js/place-explorer.js` per D-10 (the three flat `<script>`-
 ...
 * ported into the entities copy by Phase 15.2-02 per D-10:
...
// Phase 15.2-04 (CONTEXT §D-01): 4-key pivot set for /entidades/. Must
...
class EntityExplorer {
  constructor(container) {
    ...
    // Phase 15.2-04: pivot + triple sidecars, fetched in init() via
  ...
  async init() {
    ...
    // Phase 15.2-04 (CONTEXT scope row #5): parallel-fetch both pivot
    ...
    // Phase 15.2 (CONTEXT scope row #6, analog static/js/search.js:549-553):
    ...
    // by grep alone). Plan 15.2-04's pivot sidecar consumer (above)
    ...
    this.pagefind.filters()
      .then(gf => {
        ...
        // Phase 15.2-02 regression rescue (caught in post-structural
  ...
  async search() {
    ...
    try {
      if (isPreSearch) {
        ...
        // Phase 15.2-02 W2 null-guard: globalFilters may be null
      ...
      if (!this.state.q && hasActiveFilters && !this.skipBrowsePrompt && !onlyViewportActive) {
        ...
        if (estimated > 10000) {
          // Phase 15.2-04 (CONTEXT scope row #5, §D-04): when pivot /
          ...
          // overload search resolves, preserving CONTEXT §D-04 no-
          ...
          // values per entity), so rather than weaken D-04 we show a
          ...
          if (pivotScoped) {
            ...
            // total: null (D-04 no-self-contradiction invariant).
      ...
      // Phase 15.2-02 W2 null-guard: if globalFilters hasn't warmed
      ...
      // Phase 14 (D-10): replaced with selectFacetCounts per-value
  ...
  getTotalEntityCount() {
    // Phase 15.2-02 W2 null-guard: filters() now warms async, so
  ...
  estimateFilterCount() {
    // Phase 15.2-02 W2 null-guard: read through a local that is
  ...
  renderSearchResults(data) {
    ...
    //                      (Phase 15.2-04 / CONTEXT §D-04 W4 fix). The
    ...
    if (data.browsePrompt) {
      ...
      if (mode === 'overload-pending') {
        // D-04 pending-state: no numeric total. Calculando total\u2026
      ...
      continueBtn.textContent = isOverloadMode ? 'Ver todos' : 'Explorar todas';
  ...
  renderResultsInfo(data) {
    ...
    const sortOptions = [
      { field: 'name',  defaultDir: 'asc',  label: 'Nombre' },
      { field: 'date',  defaultDir: 'asc',  label: 'Fecha' },
      { field: 'count', defaultDir: 'desc', label: 'Documentos' },
    ];
  ...
  /**
   * Phase 15.2-04 (analog static/js/search.js:2275-2307): adapter
  ...
  renderSidebarFacets(containerEl, filtersArg) {
    ...
    // Phase 15.2-02 W2 null-guard: globalFilters may be null during
...
// `static/js/search.js` and `static/js/place-explorer.js` per D-10.
